...
class Swin(nn.Module):
    def __init__(self, args):
        ...
        self.proj_head = projection_head(in_dim=768, hidden_dim=1024, out_dim=3)
    def forward(self, x_in,training=True):
        ...
        out = self.swinViT(x_in)[-1]
        # out_cl = hidden_states_out[-1].contiguous()
        b= out.shape[1]
        out_re = self.proj_head(self.avg_pooling(out).reshape(-1,b))
        if not training:
            return out_re,self.avg_pooling(out).reshape(-1,b)
        return out_re
