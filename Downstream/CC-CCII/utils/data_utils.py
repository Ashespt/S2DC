...
def get_loader(args):
    ...
    def __transforms__(augmentation=True, npy=None, args=None):
        ...
        if augmentation:
            ...
            npy_normalized = resample_3d(npy_normalized,(64,384,384))
        else:
            npy_normalized = npy_normalized[
                :,
    ...
    train_files=train_files.sample(frac=args.data_ratio,random_state=1,replace=False,ignore_index=True)
    ...
    train_loader = torch.utils.data.DataLoader(
        train_ds, batch_size=args.batch_size, shuffle=False if args.distributed else True,
        num_workers=1, pin_memory=True, persistent_workers=True, sampler=DistributedSampler(train_ds, shuffle=True, seed=102) if args.distributed else None,
    )

    val_ds = CC_CCII(data=val_files, transforms=__transforms__, augmentation=False,args=args)
    print(f'=>Val len {len(val_ds)}')
    val_loader = torch.utils.data.DataLoader(
        val_ds, batch_size=1, shuffle=False, num_workers=1, pin_memory=True, persistent_workers=True,sampler=DistributedSampler(train_ds, shuffle=True, seed=102) if args.distributed else None,)
