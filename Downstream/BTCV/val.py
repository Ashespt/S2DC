...
from utils.utils import draw_mask,mask_max
...
def main():
    ...
    args.test_mode = True
    output_directory = "/cpfs01/projects-HDD/cfff-c7cd658afc74_HDD/pantan/code/cvpr/GeCo/BTCV/outputs/" + args.exp_name
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            # save results
            save_image = data.detach().cpu().numpy()[0,0,...]
            nifti_image = Nifti1Image(save_image*13751+15564, affine)
            nifti_image_out = Nifti1Image(outputs, affine)
            nifti_image_labels = Nifti1Image(val_labels, affine)
            nib.save(nifti_image, output_directory+'/'+str(idx)+'_img.nii')
            nib.save(nifti_image_out, output_directory+'/'+ str(idx)+'_out.nii')
            nib.save(nifti_image_labels, output_directory+'/'+str(idx)+'_label.nii')


            len_class = len(list(np.unique(val_labels))) - 1
            dice_list_sub = []
            slice_idx = mask_max(val_labels)
            plt.imshow(data[0,0,...].detach().cpu().numpy()[:,:,slice_idx], cmap="gray", interpolation="none")
            plt.savefig(output_directory+f'/image_{idx}.png')

            draw_mask(data[0,0,...].detach().cpu().numpy(),outputs,slice_idx,output_directory+f'/mask_image_{idx}.png')
            draw_mask(data[0,0,...].detach().cpu().numpy(),val_labels,slice_idx,output_directory+f'/mask_image_{idx}_gt.png')
            for i in range(1, 14):
